...
export function ModulesDisplay({
  ...
  const setActivePrototypeModule = useAppStore(
    (state) => state.setActivePrototypeModule
  );

  // Convert array to Set for faster lookups
  const checkedModules = useMemo(
  ...
  const modules = [
    ...
    },
    {
      ...
      subModules: [
        ...
        { id: "M04-6", name: "견적서 파일 다운로드", type: "선택" },
      ]
    },
    {
      id: "M05",
    ...
    return modules.reduce(
      (acc, mod) =>
        ...
        mod.subModules.filter(
          (sm) => sm.type === "필수" && checkedModules.has(sm.id)
        ).length,
      0
    );
  }, [checkedModules]);
  ...
  const optionalModules = useMemo(() => {
    return modules.reduce(
      (acc, mod) =>
        acc +
        mod.subModules.filter(
          (sm) => sm.type === "선택" && checkedModules.has(sm.id)
        ).length,
      0
    );
  }, [checkedModules]);
        ...
        <Card className="flex-1 border-2 shadow-md">
          <CardContent className="flex items-center justify-around p-6">
            ...
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-secondary transition-all duration-300">
                {requiredModules}개
              </p>
            </div>
            <div className="h-16 w-px bg-border" />
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-foreground/70 transition-all duration-300">
                {optionalModules}개
              </p>
            </div>
          </CardContent>
